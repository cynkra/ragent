...
And now, we also got a shiny fully-speced M4 Mac Mini for our office that runs all of this like a charm. It's local LLM time! 🥳
...
sequenceDiagram
    ...
    Agent->>RAG: Search request
    RAG->>Docs: Search in directory
    Docs-->>RAG: Relevant passages
    RAG-->>Agent: Context + passages
...
- Ask the LLM to provide additional questions to search for
- Search the knowledge base for the additional questions
